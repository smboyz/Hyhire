...
const Unskilled = () => {
  ...
  const [unskilled_1, setUnskilled_1] = useState([]);
  ...
  const AboutData = async () => {
    try {
      ...
      if (response.data) {
        const unskilledData = response.data.filter(
          (item) => item.status === "Publish" && item.page_type === "UNSKILLED"
        );
        setUnskilled(unskilledData); // Assuming you want to slice the filtered data

        const unskilled_1Data = response.data.filter(
          (item) => item.status === "Publish" && item.page_type === "SKILLED 1"
        );
        setUnskilled_1(unskilled_1Data);
      }
    } catch (error) {
      ...
      {unskilled.map((un, index) => (
        <>
          ...
          <div className="container py-16">
            <div className="flex gap-60 justify-center items-center xl:flex-row flex-col">
              ...
              <div className="xl:w-3/4 bg-gray-200 p-12 rounded-xl">
                <div className="flex gap-10 flex-wrap">
                  {unskilled_1.map((item, index) => (
                    <div className="w-72 h-80 xl:w-[234px] xl:h-80  relative">
                      <img src={item.bannerimage} alt="" className="absolute w-full h-full"/>
